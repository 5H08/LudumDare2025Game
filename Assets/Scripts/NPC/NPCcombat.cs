...
public class NPCcombat : MonoBehaviour
{
    ...
    [Header("击退")]
    public float knockBackMagnitude = 3f;      // 击退时水平移动的距离（视觉上相当于“被打飞”多远）
    ...
    public void ReceiveHit(Transform attacker)
    {
        ...
        dir.y += knockbackHeight; // 稍微向上，让击退有一点“飞起”感觉

        // 启动击退协程（控制击退动画）
        if (knockbackRunning) 
        {
            StopCoroutine(currentKnockbackCoroutine);
            currentKnockbackCoroutine = StartCoroutine(ChainedKnockback(dir.normalized * knockBackMagnitude));
        }
        else
        {
            currentKnockbackCoroutine = StartCoroutine(ApplyKnockback(dir.normalized * knockBackMagnitude));
        }
    }
